...
Based on Stage 8b with ONE change: the flow starts from a uniform
distribution (1/K) instead of the encoder's p0 output.

What changed vs Stage 8b:
  - Bridge:      pt = (1-t)*UNIFORM + t*p1    (was: (1-t)*p0 + t*p1)
  - Velocity:    vt = p1 - UNIFORM             (was: p1 - p0)
  - Integration: starts from UNIFORM           (was: starts from p0)

What is IDENTICAL to Stage 8b:
  - Encoder, CTC loss, p0 PER path: all unchanged
  - fit_batch, objectives, decoding, data pipeline: all unchanged
  - Hyperparameters: all unchanged

This gives a second comparison point: how well can the flow reconstruct
the target starting from zero information (only encoder hidden states
as conditioning) vs starting from the encoder's informed p0.
"""
...
class ASR_Brain(sb.Brain):
    ...
    def compute_forward(self, batch, stage):
        ...
        # --- UNIFORM CHANGE: flow starts from 1/K instead of p0 ---
        p_uniform = torch.full_like(p0, 1.0 / self.hparams.output_neurons)

        # Sample t, bridge, velocity: lines 10-13
        # Bridge and velocity use p_uniform instead of p0
        t = torch.rand(1, device=hidden.device).item()
        pt = self.bridge_state(p_uniform, p1, t)
        vt = self.target_velocity(p_uniform, p1)
        ...
        # Starts from p_uniform instead of p0
        if stage != sb.Stage.TRAIN:
            p_flow = self.integrate_flow(
                p_uniform, hidden, self.hparams.num_flow_steps
            )
        else:
            p_flow = None
