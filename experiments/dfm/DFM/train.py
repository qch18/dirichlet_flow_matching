...
class DFMModule(nn.Module):
    def __init__(
        ...
        # Velocity network: takes [simplex_state, encoder_state, time]
    ...
    def velocity(self, p, t, h):
        ...
        # Remove mean component to keep updates closer to simplex tangent space
    ...
    def forward(self, logits, hidden_states):
        ...
        return p, alpha0, self.dirichlet_mean(alpha0)
...
class ASR_Brain(sb.Brain):
    def compute_forward(self, batch, stage):
        ...
        hidden = self.modules.model(feats)          # [B, T, H]
        logits = self.modules.output(hidden)        # [B, T, V]

        dfm_probs, alpha0, p0 = self.modules.dfm(logits, hidden)
        log_probs = torch.log(dfm_probs.clamp_min(1e-8))
    ...
    def compute_objectives(self, predictions, batch, stage):
        ...
        # optional light regularisation so flow stays near initial simplex early on
        p0 = predictions["p0"]
        dfm_probs = predictions["dfm_probs"]
        reg_loss = F.mse_loss(dfm_probs, p0)

        loss = ctc_loss + self.hparams.dfm_reg_weight * reg_loss
